function [loss,class_probs,class_probs2] = softmax(output_dimension,input,N,labels,lambda,W1,W2,W3)
...
reg_loss = 0.5 * lambda * sum(sum(W1.*W1)) + 0.5 * lambda * sum(sum(W2.*W2)) + 0.5 * lambda * sum(sum(W3.*W3));
